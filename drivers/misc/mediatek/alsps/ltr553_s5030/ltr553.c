...
#define FEATURE_IR_CURRENT_SET
#endif
#ifdef FEATURE_IR_CURRENT_SET
static int g_ps_base_value_LED_CURRENT = 0;
static int g_ps_first_LED_CURRENT = 0;
#endif
/******************************************************************************
 * extern functions
*******************************************************************************/
...
/*----------------------------------------------------------------------------*/
#ifdef FEATURE_IR_CURRENT_SET
int  ltr553_get_current_value(void)
{
u8 value=0;
		 if (hwmsen_read_byte_sr(ltr553_obj->client, APS_RW_PS_LED, &value))
    			{
		        printk("reads aps data = %d\n", value);
		        return -EFAULT;
		    	}
		 g_ps_first_LED_CURRENT=value;
		 return 0;
}
#endif
...
#ifdef FEATURE_IR_CURRENT_SET
static void ltr553_reset_led_current(int led_current)
{
	led_current&=0xff0000;
	led_current>>=16;
	APS_LOG("ltr553_reset_led_current:set g_ps_base_value_LED_CURRENT =%x,led_current=%x \n",g_ps_base_value_LED_CURRENT, led_current);
	hwmsen_write_byte(ltr553_obj->client, APS_RW_PS_LED, led_current);   
}
#endif
static void ps_cali_set_threshold(void)
{
...
#ifdef FEATURE_IR_CURRENT_SET
	ltr553_reset_led_current(g_ps_base_value_LED_CURRENT);
#endif
    ltr553_ps_set_thres();
}
...
static void ps_cali_start(void)
{
    ...
    */
#ifdef FEATURE_IR_CURRENT_SET
	hwmsen_write_byte(ltr553_obj->client, APS_RW_PS_LED, g_ps_first_LED_CURRENT);   
mdelay(30);
IR_CURRENT_SET_START:
vl_ps_count = 0;
vl_ps_sun = 0;
#endif 
	...
	uint8_t value_set;
   if((g_ps_base_value+hw->ps_threshold_high)>(0x780))      //adjust the value of IR current
   	{
		 if (hwmsen_read_byte_sr(ltr553_obj->client, APS_RW_PS_LED, &value))
    			{
		        printk("reads aps data = %d\n", value);
		...
		value_set = value&0x07;
		if(value_set==0)         //the minimun value of the IR current
			{goto IR_CURRENT_SET_END;}

		value--;
 		hwmsen_write_byte(ltr553_obj->client, APS_RW_PS_LED, value);   

		//g_ps_base_value_LED_CURRENT=g_ps_base_value|((0x000000+value)<<16);
		 APS_LOG("ps_cali_start:FEATURE_IR_CURRENT_SET: g_ps_base_value_LED_CURRENT=%x \n",g_ps_base_value_LED_CURRENT);
		mdelay(10);
		goto IR_CURRENT_SET_START;
   ...
   IR_CURRENT_SET_END:
		...
		g_ps_base_value_LED_CURRENT=value;
		g_ps_base_value_LED_CURRENT<<=16;
		g_ps_base_value_LED_CURRENT|=(g_ps_base_value&0x7ff);
...
static long ltr553_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
    ...
    switch (cmd)
    {
		...
		#ifndef FEATURE_IR_CURRENT_SET
            ps_cali_data[1] = g_ps_base_value;
		#endif

		#ifdef FEATURE_IR_CURRENT_SET
            ps_cali_data[1] = g_ps_base_value_LED_CURRENT;
		#endif
		...
		#ifdef FEATURE_IR_CURRENT_SET
            	g_ps_base_value_LED_CURRENT= ps_cali_data[1];
		g_ps_base_value=ps_cali_data[1]&0x7ff;
		if(0==(g_ps_base_value_LED_CURRENT&0xff0000))
			g_ps_base_value_LED_CURRENT|=(g_ps_first_LED_CURRENT<<16);
		#endif
            if(!g_ps_cali_flag)
            {
